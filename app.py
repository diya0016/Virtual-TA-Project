...
async def get_embedding(text, max_retries=3):
    ...
    while retries < max_retries:
        try:
            logging.info(f"Getting embedding for text (length: {len(text)})")
            ...
            logging.info("Sending request to embedding API")
...
async def find_similar_content(query_embedding, conn):
    try:
        logging.info("Finding similar content in database")
        cursor = conn.cursor()
        results = []
        
        # Search discourse chunks
        logging.info("Querying discourse chunks")
        cursor.execute("""
        ...
        logging.info(f"Processing {len(discourse_chunks)} discourse chunks")
        processed_count = 0
        ...
        for chunk in discourse_chunks:
            try:
                ...
                if processed_count % 1000 == 0:
                    logging.info(f"Processed {processed_count}/{len(discourse_chunks)} discourse chunks")
                    
            except Exception as e:
                logger.error(f"Error processing discourse chunk {chunk['id']}: {e}")
        
        # Search markdown chunks
        logging.info("Querying markdown chunks")
        cursor.execute("""
        ...
        logging.info(f"Processing {len(markdown_chunks)} markdown chunks")
        processed_count = 0
        ...
        for chunk in markdown_chunks:
            try:
                ...
                processed_count += 1
                if processed_count % 1000 == 0:
                    logging.info(f"Processed {processed_count}/{len(markdown_chunks)} markdown chunks")
        ...
        logging.info(f"Found {len(results)} relevant results above threshold")
        ...
        #edit
        if not results:
            logger.warning("⚠️ No chunks passed the similarity threshold.")

        else:
            logging.info(f"Top {len(results)} raw matches above threshold:")
        for r in results[:5]:
            logging.info(f"✓ Similarity: {r['similarity']:.4f} | Source: {r['source']} | Title: {r.get('title', 'N/A')} | Preview: {r['content'][:80].replace('\n', ' ')}...")

        # Return top results, limited by MAX_RESULTS
        logging.info(f"Returning {len(final_results[:MAX_RESULTS])} final results after grouping")
...
async def enrich_with_adjacent_chunks(conn, results):
    try:
        logging.info(f"Enriching {len(results)} results with adjacent chunks")
        cursor = conn.cursor()
        enriched_results = []
        ...
        logging.info(f"Successfully enriched {len(enriched_results)} results")
...
async def generate_answer(question, relevant_results, max_retries=2):
    ...
    while retries < max_retries:    
        try:
            ...
            logging.info(f"Built prompt context length: {len(context)} chars")
            logger.debug(f"Context preview:\n{context[:500]}")
            

            logging.info(f"FULL CONTEXT PASSED TO LLM:\n{context}")
            ...
            logging.info(f"Final Prompt Sent:\n{prompt}")


            logging.info(f"FULL LLM PROMPT SENT:\n{prompt}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload) as response:
                    if response.status == 200:
                        result = await response.json()
                        logging.info("Successfully received answer from LLM")
...
async def process_multimodal_query(question, image_base64):
    ...
    try:
        logging.info(f"Processing query: '{question[:50]}...', image provided: {image_base64 is not None}")
        if not image_base64:
            logging.info("No image provided, processing as text-only query")
            return await get_embedding(question)
        
        logging.info("Processing multimodal query with image")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, headers=headers, json=payload) as response:
                if response.status == 200:
                    ...
                    logging.info(f"Received image description: '{image_description[:50]}...'")
                    ...
                    logging.info("Falling back to text-only query")
                    return await get_embedding(question)
    except Exception as e:
        logger.error(f"Exception processing multimodal query: {e}")
        logger.error(traceback.format_exc())
        # Fall back to text-only query
        logging.info("Falling back to text-only query due to exception")
        return await get_embedding(question)
...
def parse_llm_response(response):
    try:
        ...
        logging.info(f"Parsed answer (length: {len(answer)}) and {len(links)} sources")
...
async def query_knowledge_base(request: QueryRequest):
    try:
        ...
        logging.info("DEBUG: Query endpoint reached!")
        logging.info(f"Received query request: question='{request.question[:50]}...', image_provided={request.image is not None}")
        ...
        try:
            ...
            logging.info("Processing query and generating embedding")
            ...
            logging.info(f"Retrieved {len(relevant_results)} relevant chunks")
            for i, res in enumerate(relevant_results[:3]):
                logging.info(f"Chunk {i+1}: URL={res['url']}, Content Preview={res['content'][:100].replace('\n',' ')}...")
                logging.info(f"{i+1}. Score: {res.get('similarity', 'N/A'):.3f} | {res['content'][:100]}...")
            ...
            logging.info("Enriching results with adjacent chunks")
            ...
            if not result["links"]:
                logging.info("No links extracted, creating from relevant results")
            ...
            logging.info(f"Returning result: answer_length={len(result['answer'])}, num_links={len(result['links'])}")
