...
def process_discourse_files(conn):
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for post in posts:
                    ...
                    for i, chunk in enumerate(chunks):
                        cursor.execute('''SELECT 1 FROM discourse_chunks WHERE post_id = ? AND chunk_index = ?''', (post_id, i))
                        ...
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL)
                        ''', (post_id, topic_id, topic_title, post_number, author, created_at, likes, i, chunk, url))
                        new_chunks += 1
    ...
    logger.info(f"Finished processing Discourse files. Created {new_chunks} chunks.")
...
def process_markdown_files(conn):
    ...
    for file_name in tqdm(markdown_files, desc="Processing Markdown files"):
        try:
            ...
            with open(file_path, 'r', encoding='utf-8') as file:
                ...
                for i, chunk in enumerate(chunks):
                    cursor.execute('''
                        SELECT embedding FROM markdown_chunks 
                        WHERE doc_title = ? AND chunk_index = ?
                    ''', (title, i))
                    row = cursor.fetchone()

                    if row is None:
                        # Insert new chunk with embedding NULL
                        cursor.execute('''
    ...
    logger.info(f"Processed {len(markdown_files)} files. New chunks added: {total_new_chunks}")
                    ...
                    if response.status == 200:
                        ...
                        if part_id:
                            ...
                            if is_discourse:
                                ...
                                cursor.execute("""
                                SELECT post_id, topic_id, topic_title, post_number, author, created_at, likes, chunk_index, content, url FROM discourse_chunks 
                                WHERE id = ?
                                """, (record_id,))
...
async def main():
    ...
    logger.info("✅ All files processed and embeddings created successfully.")
